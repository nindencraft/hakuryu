...
export async function listarCargosPainel(gangId: number): Promise<CargoPainelPersonalizado[]> {
  const { data, error } = await getDb()
    ...
    .select("id, nome, discord_role_id, permissoes, criado_em")
    .eq("gang_id", gangId)
...
export async function permissoesDoUsuario(
  ...
  if (acessoTotal) return [...CHAVES_PERMISSOES_PAINEL];
...
export async function salvarCargoPainel(
  ...
  const { data, error } = await query.select("id, nome, discord_role_id, permissoes, criado_em").single();
