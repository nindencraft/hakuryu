...
async function gangsAtivas(): Promise<GangLinha[]> {
  ...
  const { data, error } = await db.from("gangs").select("id, nome, guild_id, ativo").eq("ativo", true).order("nome");
...
async function relacoesDaGang(gangId: number): Promise<{
  relacoes: RelacaoLinha[];
...
async function limparRelacao(gangId: number, outraGangId: number) {
  ...
  const { error } = await db.from("gang_relacoes").delete().eq("gang_a_id", a).eq("gang_b_id", b);

  if (error && !ausente(error)) {
    throw new Error(error.message);
  }
}
...
export async function listarSolicitacoes(user: SessionUser): Promise<{
...
export async function listarGuerrasAtivas(user: SessionUser): Promise<{
  guerras: GuerraAtiva[];
  tabelaAusente: boolean;
}> {
  const db = getDb();
...
export async function criarSolicitacao(user: SessionUser, input: NovaSolicitacao) {
  ...
  if (input.gangId === minha) {
    throw new Error("Você não pode se solicitar.");
  }
...
export async function responderSolicitacao(
  ...
  const { data, error } = await db.from("gang_solicitacoes").select("*").eq("id", input.id).maybeSingle();
  ...
  const { error: upErr } = await db.from("gang_solicitacoes").update(patch).eq("id", sol.id);
...
async function criarTreinosAmistosos(
  ...
  if (error) {
    throw new Error(`Não consegui criar o treino amistoso: ${error.message}`);
  }

  const criados = (data ?? []) as {
    id_treino: number;
    gang_id: number;
  }[];
...
export async function encerrarGuerra(user: SessionUser, input: { id: number }) {
  ...
  const colunaAusente =
    !!marcaErr &&
    /encerrar_(origem|destino)|column .* does not exist|PGRST204/i.test(
      `${marcaErr.code ?? ""} ${marcaErr.message ?? ""}`,
    );

  if (marcaErr && !colunaAusente) {
    throw new Error(marcaErr.message);
  }
